...
real*8,dimension(:),allocatable :: E,B,Z,t0,phaseR,phaseI
...
do j = 1, N
    ...
    do i = 1, ntime + 1
        ...
        IDCTI = 0.d0
        do m = 1, N
            phaseR(m) = cos(E(m)*t0(i))
            phaseI(m) = sin(E(m)*t0(i))
            IDCTR = IDCTR + coeff(m,j)*phaseR(m)
            IDCTI = IDCTI + coeff(m,j)*phaseI(m)
        end do
    ...
    fidel = temp
	open(1,access='append',file='bose3.txt')
    if (j .eq. 1) then
        write(*,*) '          N', '     fidelity'
		write(1,*) '          N', '     fidelity'
    end if
