!Check fidelity at every site at different values of N
!Numerical Solution
program main    
implicit none
logical EV
real*8 magnet,Jint,tend,IDCTR,IDCTI,fidel,concu,temp,dt
real*8 phaseR,phaseI,t0,t1,t2
integer IROT,i,m,ntime,j,N
real*8,dimension(:),allocatable :: E,B,Z
real*8,dimension(:,:),allocatable :: A,V,coeff
!
write(*,*) 'N = ?'
read(*,*) N
!
...
do j = 1, N
    ...
    do i = 1, ntime + 1
        ...
        do m = 1, N
            phaseR = cos(E(m)*t0)
            phaseI = sin(E(m)*t0)
            IDCTR = IDCTR + coeff(m,j)*phaseR
            IDCTI = IDCTI + coeff(m,j)*phaseI
        end do
        fidel = sqrt(IDCTR**2 + IDCTI**2)/3.d0 + (IDCTR**2 + IDCTI**2)/6.d0 + 0.5d0
        if (fidel .gt. temp) then
            temp = fidel
        end if
    end do
    fidel = temp
	!open(1,access='append',file='22222.txt')
    if (j .eq. 1) then
        write(*,*) '          N', '     fidelity'
		!write(1,*) '          N', '     fidelity'
    end if
